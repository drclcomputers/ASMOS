...
static void double_to_str(double v, char *buf, int max) {
    ...
    for (int i = 0; i < ti && idx < max - 1; i++) buf[idx++] = tmp[i];
...
static void calc_set_display(calc_state_t *s, const char *str) {
    int i = 0;
    while (str[i] && i < MAX_DISPLAY) { s->display[i] = str[i]; i++; }
    s->display[i] = '\0';
    s->display_len = i;
}
...
static double calc_parse(const char *buf) {
    ...
    int sign = 1;
    if (buf[i] == '-') { sign = -1; i++; }
    while (buf[i] >= '0' && buf[i] <= '9') { result = result * 10.0 + (buf[i] - '0'); i++; }
    if (buf[i] == '.') {
        i++;
        double frac = 0.1;
        while (buf[i] >= '0' && buf[i] <= '9') { result += (buf[i] - '0') * frac; frac *= 0.1; i++; }
    }
    return result * sign;
}
...
static void calc_apply_op(calc_state_t *s) {
    ...
    switch (s->pending_op) {
        ...
        case '*': r = a * b; break;
        case '/':
            if (b == 0.0) { s->error = true; calc_set_display(s, "Error"); return; }
            r = a / b;
            break;
        default: r = b; break;
    }
...
static void calc_on_button(calc_state_t *s, const char *lbl) {
    if (s->error && lbl[0] != 'C') return;

    if (lbl[0] == 'C') { calc_reset(s); return; }
    ...
    if (lbl[0] == '+' && lbl[1] == '/') {
        if (s->display[0] == '-') {
            ...
            buf[0] = '-';
            int i = 0;
            while (s->display[i]) { buf[i + 1] = s->display[i]; i++; }
            buf[i + 1] = '\0';
            calc_set_display(s, buf);
        }
        return;
    ...
    if (lbl[0] == '+' || lbl[0] == '-' || lbl[0] == '*' || lbl[0] == '/') {
        if (s->pending_op && !s->fresh_operand) calc_apply_op(s);
    ...
    if (lbl[0] == '=') {
        if (s->pending_op) calc_apply_op(s);
        s->pending_op = 0;
        return;
    }
...
static void calc_draw(window *win, void *ud) {
    ...
    if (txt_x < wx + DISPLAY_X + 2) txt_x = wx + DISPLAY_X + 2;
    draw_string(txt_x, wy + DISPLAY_Y + 4, s->display, s->error ? RED : WHITE, 2);
...
static void calc_on_frame(void *state) {
    ...
    if (mouse.left_clicked) {
        ...
        if (hit >= 0) calc_on_button(s, BUTTONS[hit].label);
    }
    ...
    if (kb.key_pressed) {
        ...
        } else if (c == '+') { calc_on_button(s, "+"); }
        else if (c == '-')   { calc_on_button(s, "-"); }
        else if (c == '*')   { calc_on_button(s, "*"); }
        else if (c == '/')   { calc_on_button(s, "/"); }
        else if (c == '.')   { calc_on_button(s, "."); }
        else if (c == '%')   { calc_on_button(s, "%"); }
        else if (kb.last_scancode == ENTER) { calc_on_button(s, "="); }
        ...
        } else if (kb.last_scancode == ESC) { calc_reset(s); }
    }
