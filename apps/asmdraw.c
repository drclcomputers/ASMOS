...
static const uint8_t PALETTE_COLORS[PALETTE_COLS] = {
    ...
    RED, LIGHT_RED, GREEN, LIGHT_GREEN,
    BLUE, LIGHT_BLUE, YELLOW, LIGHT_YELLOW,
...
static void draw_set_status(asmdraw_state_t *s, const char *msg) {
    strncpy(s->status, msg, 47); s->status[47] = '\0';
    s->status_timer = 240;
}

static bool mouse_to_canvas(const asmdraw_state_t *s, int mx, int my, int *cx_out, int *cy_out) {
    int wx  = s->win->x + 1;
    ...
    if (cx < 0 || cy < 0 || cx >= s->canvas_w || cy >= canvas_px_h || cy >= s->canvas_h)
        return false;
...
static void canvas_paint(asmdraw_state_t *s, int cx, int cy, uint8_t color) {
    ...
    if (lo_x < 0) lo_x = 0;
    if (lo_y < 0) lo_y = 0;
    if (hi_x >= s->canvas_w)  hi_x = s->canvas_w  - 1;
    if (hi_y >= s->canvas_h)  hi_y = s->canvas_h  - 1;
    for (int y = lo_y; y <= hi_y; y++)
        for (int x = lo_x; x <= hi_x; x++)
            s->canvas[y][x] = color;
}

static void canvas_line(asmdraw_state_t *s, int x0, int y0, int x1, int y1, uint8_t color) {
...
static void canvas_fill(asmdraw_state_t *s, int cx, int cy, uint8_t color) {
    ...
    while (top > 0) {
        ...
        int x = stack[top].x;
        int y = stack[top].y;
        if (x < 0 || y < 0 || x >= s->canvas_w || y >= s->canvas_h) continue;
        if (s->canvas[y][x] != target) continue;
        s->canvas[y][x] = color;
        if (top + 4 < (int)(sizeof(stack) / sizeof(stack[0]))) {
            stack[top].x = (int16_t)(x+1); stack[top].y = (int16_t)y; top++;
            stack[top].x = (int16_t)(x-1); stack[top].y = (int16_t)y; top++;
            stack[top].x = (int16_t)x;     stack[top].y = (int16_t)(y+1); top++;
            stack[top].x = (int16_t)x;     stack[top].y = (int16_t)(y-1); top++;
        }
    }
}

static void canvas_rect(asmdraw_state_t *s, int x0, int y0, int x1, int y1, uint8_t color) {
...
static void asmdraw_draw(window *win, void *ud) {
    ...
    for (int b = 0; b < BRUSH_SIZES; b++) {
        int bx = wx + TOOLBAR_PAD;
        int by = wy + TOOLBAR_PAD + (NUM_TOOLS + 1 + b) * (TOOL_BTN_SZ + TOOLBAR_PAD);
    ...
    int sw_y = wy + TOOLBAR_PAD + (NUM_TOOLS + BRUSH_SIZES + 2) * (TOOL_BTN_SZ + TOOLBAR_PAD);
    fill_rect(sw_x + 3, sw_y + 3, TOOL_BTN_SZ - 3, TOOL_BTN_SZ - 3, s->bg_color);
    draw_rect(sw_x + 3, sw_y + 3, TOOL_BTN_SZ - 3, TOOL_BTN_SZ - 3, BLACK);
    fill_rect(sw_x,     sw_y,     TOOL_BTN_SZ - 3, TOOL_BTN_SZ - 3, s->fg_color);
    draw_rect(sw_x,     sw_y,     TOOL_BTN_SZ - 3, TOOL_BTN_SZ - 3, BLACK);*/
...
static void asmdraw_save(asmdraw_state_t *s, const char *path) {
    ...
    fat16_file_t f;
    if (!fat16_create(path, &f)) { draw_set_status(s, "Save failed."); return; }
...
static void asmdraw_load(asmdraw_state_t *s, const char *path) {
    fat16_file_t f;
    if (!fat16_open(path, &f)) { draw_set_status(s, "File not found."); return; }

    uint8_t hdr[4];
    if (fat16_read(&f, hdr, 4) < 4) { fat16_close(&f); draw_set_status(s, "Bad file."); return; }
...
static void commit_fname(asmdraw_state_t *s) {
    ...
    for (int i = 0; i < s->fname_len; i++)
        if (s->fname_buf[i] == '.') { has_dot = true; break; }
    ...
    s->fname_mode = 0;
    if (mode == 1) asmdraw_save(s, s->fname_buf);
    else           asmdraw_load(s, s->fname_buf);
}
...
static asmdraw_state_t *active_asmdraw(void) {
    ...
    for (int i = 0; i < MAX_RUNNING_APPS; i++) {
        ...
        if (!a->running || a->desc != &asmdraw_app) continue;
...
static void menu_new(void) {
    asmdraw_state_t *s = active_asmdraw(); if (!s) return;
    for (int y = 0; y < CANVAS_MAX_H; y++)
        for (int x = 0; x < CANVAS_MAX_W; x++)
            s->canvas[y][x] = WHITE;
...
static void on_about_draw(void) {
    modal_show(MODAL_INFO, "About ASMDraw",
               "ASMDraw v1.0\nASMOS Paint App", NULL, NULL);
}
...
static int toolbar_hit_tool(const asmdraw_state_t *s, int mx, int my) {
    ...
    for (int t = 0; t < NUM_TOOLS; t++) {
        int bx = wx + TOOLBAR_PAD;
        int by = wy + TOOLBAR_PAD + t * (TOOL_BTN_SZ + TOOLBAR_PAD);
        if (mx >= bx && mx < bx + TOOL_BTN_SZ &&
            my >= by && my < by + TOOL_BTN_SZ)
            return t;
    }
    return -1;
}
...
static int palette_hit(const asmdraw_state_t *s, int mx, int my) {
    ...
    if (my < pal_y || my >= pal_y + PALETTE_H) return -1;
    int col = (mx - pal_x0) / cell_w;
    if (col < 0 || col >= PALETTE_COLS) return -1;
    return col;
}
...
static void asmdraw_on_frame(void *state) {
    ...
    if (mouse.left_clicked) {
        ...
        if (t >= 0) { s->tool = t; asmdraw_draw(s->win, s); return; }

        int b = toolbar_hit_brush(s, mouse.x, mouse.y);
        if (b >= 0) { s->brush_size = b; asmdraw_draw(s->win, s); return; }

        int p = palette_hit(s, mouse.x, mouse.y);
        if (p >= 0) {
            if (kb.shift) s->bg_color = PALETTE_COLORS[p];
            else          s->fg_color = PALETTE_COLORS[p];
            asmdraw_draw(s->win, s);
            return;
        }
    }
    if (mouse.right_clicked) {
        int p = palette_hit(s, mouse.x, mouse.y);
        if (p >= 0) { s->bg_color = PALETTE_COLORS[p]; asmdraw_draw(s->win, s); return; }
    }
