...
static void settings_draw(window *win, void *ud) {
    ...
    draw_string(wx + LBL_X, ROW_Y(row), "Wallpaper:", DARK_GRAY, 2); row++;
    ...
    fill_rect(wx + VAL_X,    ROW_Y(row)-1, 16, 9, g_cfg.wallpaper_secondary_color);
    draw_rect(wx + VAL_X,    ROW_Y(row)-1, 16, 9, BLACK);
    btn(wx + VAL_X+18,   ROW_Y(row)-1, 10, 9, "-", DARK_GRAY);
    btn(wx + VAL_X+30,   ROW_Y(row)-1, 10, 9, "+", DARK_GRAY);
    row++;
    ...
    draw_string(wx + VAL_X, ROW_Y(row),
                g_cfg.play_bootchime ? "ON" : "OFF", BLACK, 2);
    ...
    draw_string(wx + VAL_X, ROW_Y(row),
                g_cfg.sound_enabled ? "ON" : "OFF", BLACK, 2);
    btn(wx + VAL_X+20,  ROW_Y(row)-1, 24, 9,
    ...
    draw_string(wx + VAL_X, ROW_Y(row),
                g_cfg.start_in_gui ? "YES" : "NO", BLACK, 2);
...
static void settings_on_frame(void *state) {
    ...
    if (mouse.left_clicked) {
        ...
        if (btn(wx+VAL_X+18, ROW_Y(row)-1, 10, 9, "-", DARK_GRAY)) {
            if (g_cfg.wallpaper_main_color > 0) { g_cfg.wallpaper_main_color--; s->dirty = true; }
        }
        if (btn(wx+VAL_X+30, ROW_Y(row)-1, 10, 9, "+", DARK_GRAY)) {
            if (g_cfg.wallpaper_main_color < 15) { g_cfg.wallpaper_main_color++; s->dirty = true; }
        }
        row++;

        if (btn(wx+VAL_X+18, ROW_Y(row)-1, 10, 9, "-", DARK_GRAY)) {
            if (g_cfg.wallpaper_secondary_color > 0) { g_cfg.wallpaper_secondary_color--; s->dirty = true; }
        }
        if (btn(wx+VAL_X+30, ROW_Y(row)-1, 10, 9, "+", DARK_GRAY)) {
            if (g_cfg.wallpaper_secondary_color < 15) { g_cfg.wallpaper_secondary_color++; s->dirty = true; }
        }
        row++;

        if (btn(wx+VAL_X+20, ROW_Y(row)-1, 10, 9, "-", DARK_GRAY)) {
            int8_t tz = (int8_t)g_cfg.timezone_offset;
            if (tz > -12) { g_cfg.timezone_offset = (uint8_t)(tz - 1); s->dirty = true; }
        }
        if (btn(wx+VAL_X+32, ROW_Y(row)-1, 10, 9, "+", DARK_GRAY)) {
            int8_t tz = (int8_t)g_cfg.timezone_offset;
            if (tz < 14) { g_cfg.timezone_offset = (uint8_t)(tz + 1); s->dirty = true; }
        }
        row++;

        if (btn(wx+VAL_X+20, ROW_Y(row)-1, 24, 9,
                g_cfg.play_bootchime ? "Off" : "On", DARK_GRAY)) {
            g_cfg.play_bootchime ^= 1; s->dirty = true;
        }
        row++;

        if (btn(wx+VAL_X+20, ROW_Y(row)-1, 24, 9,
                g_cfg.sound_enabled ? "Off" : "On", DARK_GRAY)) {
            g_cfg.sound_enabled ^= 1; s->dirty = true;
        }
        row++;

        if (btn(wx+VAL_X+22, ROW_Y(row)-1, 24, 9,
                g_cfg.start_in_gui ? "No" : "Yes", DARK_GRAY)) {
            g_cfg.start_in_gui ^= 1; s->dirty = true;
        }
        row += 2;
