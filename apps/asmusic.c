...
static int win_cy(const asmusic_state_t *s) { return s->win->y + MENUBAR_H + 16; }

static bool grid_hit(const asmusic_state_t *s, int mx, int my, int *row, int *col) {
    int ox = win_cx(s) + GRID_X;
    int oy = win_cy(s) + GRID_Y;
    int rx = mx - ox;
    int ry = my - oy;
    if (rx < 0 || ry < 0 || rx >= GRID_W || ry >= GRID_H) return false;
    *col = rx / CELL_W;
    *row = ry / CELL_H;
    return true;
}
...
static void asmusic_draw(window *win, void *ud) {
    ...
    if (s->status_timer > 0) {
        ...
        draw_string(cx + 2, cy + STATUS_Y, s->playing ? "playing..." : "click cells to draw a melody", DARK_GRAY, 2);
    }
}
...
static void asmusic_tick_playback(asmusic_state_t *s) {
    ...
    s->tick_accum++;
    if (s->tick_accum < s->ticks_per_step) return;
    s->tick_accum = 0;
...
static asmusic_state_t *active_asmusic(void) {
    ...
    for (int i = 0; i < MAX_RUNNING_APPS; i++) {
        ...
        if (!a->running || a->desc != &asmusic_app) continue;
...
static void menu_play_stop(void) {
    asmusic_state_t *s = active_asmusic(); if (!s) return;
    s->playing   = !s->playing;
    s->cur_col   = -1;
    s->tick_accum = 0;
    if (!s->playing) { speaker_tone_stop(); s->note_on = false; }
}
