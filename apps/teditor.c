...
static void np_reindex(teditor_state_t *s) {
    ...
    for (int i = 0; i < s->text_len && s->line_count < MAX_LINES; i++) {
        if (s->text[i] == '\n') {
            if (cur_len > s->max_line_len) s->max_line_len = cur_len;
            cur_len = 0;
...
static int np_line_of(teditor_state_t *s, int pos) {
    ...
    while (lo < hi) {
        int mid = (lo + hi + 1) / 2;
        if (s->line_start[mid] <= pos) lo = mid;
        else hi = mid - 1;
    }
    return lo;
}
...
static void np_scroll_to_cursor(teditor_state_t *s) {
    ...
    if (s->scroll_line < 0) s->scroll_line = 0;

    int col = np_col_of(s, s->cursor);
    if (col < s->scroll_col)
        s->scroll_col = col;
    if (s->scroll_col < 0) s->scroll_col = 0;
}

static void np_insert(teditor_state_t *s, const char *bytes, int len) {
    if (s->text_len + len >= TEXT_CAP) { np_set_status(s, "Buffer full!"); return; }
    memmove(s->text + s->cursor + len,
            s->text + s->cursor,
            s->text_len - s->cursor);
...
static bool np_save(teditor_state_t *s, const char *path) {
    ...
    fat16_close(&f);
    if (written < s->text_len) return false;
    s->dirty = false;
    return true;
}
...
static void menu_new(void) {
    teditor_state_t *s = active_teditor(); if (!s) return;
    s->text[0]     = '\0';
    s->text_len    = 0;
    s->cursor      = 0;
    s->scroll_col  = 0;
...
static void on_about_np(void) {
    modal_show(MODAL_INFO, "About TEditor",
               "TEditor v1.1\nASMOS Text Editor",
               NULL, NULL);
}

static void get_layout(const teditor_state_t *s,
                       int *text_x, int *text_y,
                       int *text_w, int *text_h,
                       int *vsb_x,  int *vsb_y, int *vsb_h,
                       int *hsb_x,  int *hsb_y, int *hsb_w,
                       int *status_y) {
    int wx = s->win->x + 1;
...
static void np_draw(window *win, void *ud) {
    ...
    int status_y;
    get_layout(s, &text_x, &text_y, &text_w, &text_h,
               &vsb_x, &vsb_y, &vsb_h,
    ...
    if (s->fname_mode != FNAME_MODE_NONE) {
        ...
        const char *dlg_title = (s->fname_mode == FNAME_MODE_OPEN)
                                ? "Open File" : "Save As";
        ...
        if (field_chars < 1) field_chars = 1;
...
static bool teditor_close_cb(window *w) {
    for (int i = 0; i < MAX_RUNNING_APPS; i++) {
        ...
        if (!a->running || a->desc != &teditor_app) continue;
        teditor_state_t *s = (teditor_state_t *)a->state;
        if (s->win == w) { os_quit_app(a); return true; }
    }
    return true;
}
...
static void changecursor(void *state) {
    ...
    if(mouse.x >= s->win->x && mouse.x <= s->win->x + s->win->w - SCROLLBAR_W &&
    	mouse.y >= s->win->y + TITLEBAR_H && mouse.y < s->win->y + s->win->h - SCROLLBAR_H)
    	CHOSEN_CURSOR = 3;
    else
    	CHOSEN_CURSOR = 0;
}
...
static void teditor_on_frame(void *state) {
    ...
    if ((mouse.left_clicked || mouse.left) &&
        mouse.x >= vsb_x && mouse.x < vsb_x + SCROLLBAR_W &&
        mouse.y >= vsb_y && mouse.y < vsb_y + vsb_h &&
    ...
    if ((mouse.left_clicked || mouse.left) &&
        mouse.x >= hsb_x && mouse.x < hsb_x + hsb_w &&
        mouse.y >= hsb_y && mouse.y < hsb_y + SCROLLBAR_H &&
        s->max_line_len > max_vis_chars) {
        int max_h_scroll = s->max_line_len - max_vis_chars;
        int track_w      = hsb_w - 4;
        int new_scroll   = ((mouse.x - hsb_x - 2) * max_h_scroll) / track_w;
        if (new_scroll < 0) new_scroll = 0;
        if (new_scroll > max_h_scroll) new_scroll = max_h_scroll;
        s->scroll_col = new_scroll;
    }

    if (mouse.left_clicked &&
        mouse.x >= text_x && mouse.x < text_x + text_w &&
        mouse.y >= text_y && mouse.y < text_y + text_h) {
        int row = (mouse.y - text_y) / LINE_H;
        int ln  = s->scroll_line + row;
        if (ln >= s->line_count) ln = s->line_count - 1;
        if (ln < 0) ln = 0;
    ...
    if (kb.key_pressed) {
        ...
        if (sc == ENTER) {
            ...
            if (s->cursor > 0) { s->cursor--; np_scroll_to_cursor(s); }
        } else if (sc == RIGHT_ARROW) {
            if (s->cursor < s->text_len) { s->cursor++; np_scroll_to_cursor(s); }
        } else if (sc == HOME) {
            int ln = np_line_of(s, s->cursor);
            ...
            s->scroll_line += s->visible_rows;
            if (s->scroll_line > max_scroll) s->scroll_line = max_scroll;
            int bot = s->scroll_line + s->visible_rows - 1;
            if (bot >= s->line_count) bot = s->line_count - 1;
