...
fat16_dir_context_t dir_context = {.current_cluster = 0, .path = "/"};
...
static uint16_t fat_get(uint16_t cluster) {
    ...
    if (!ata_read_sector(fat_sector, local_buf))
        return FAT16_BAD;
...
static bool fat_set(uint16_t cluster, uint16_t value) {
    ...
    for (int copy = 0; copy < fs.bpb.fat_count; copy++) {
        ...
        if (!ata_read_sector(lba, local_buf))
            return false;
        *(uint16_t *)(local_buf + fat_offset) = value;
        if (!ata_write_sector(lba, local_buf))
            return false;
    }
    return true;
}
...
static bool dir_iterate(uint16_t dir_cluster, dir_cb cb, void *user) {
    ...
    if (dir_cluster == 0) {
        ...
        for (uint32_t s = 0; s < root_sectors; s++) {
            lba = fs.root_lba + s;
            if (!ata_read_sector(lba, sector_buf))
                return false;
...
static bool cluster_iterate(uint16_t cluster, cluster_cb cb, void *user) {
    while (cluster >= 2 && cluster < FAT16_RESERVED) {
        uint32_t start_lba = cluster_to_lba(cluster);
        for (uint32_t s = 0; s < fs.bpb.sectors_per_cluster; s++) {
            if (!ata_read_sector(start_lba + s, sector_buf))
                return false;
            dir_entry_t *entries = (dir_entry_t *)sector_buf;
            for (int i = 0; i < 16; i++) {
...
bool fat16_mount(void) {
    ...
    uint32_t total = fs.bpb.total_sectors_16 ? fs.bpb.total_sectors_16
...
bool fat16_get_usage(uint32_t *total_bytes, uint32_t *used_bytes) {
    ...
    *total_bytes = (fs.bpb.total_sectors_32 ? fs.bpb.total_sectors_32
    ...
    for (uint32_t s = 0; s < fs.bpb.sectors_per_fat; s++) {
        if (!ata_read_sector(fs.fat_lba + s, fat_buf))
            return false;
        for (int i = 0; i < 256; i++) {
...
void fat16_wipe_cluster(uint16_t cluster) {
    ...
    while (curr >= 2 && curr < FAT16_RESERVED) {
        ...
        for (uint32_t s = 0; s < fs.bpb.sectors_per_cluster; s++) {
            uint8_t local_buf[512];
            if (!ata_read_sector(start_lba + s, local_buf))
                return;
...
bool fat16_create(const char *path, fat16_file_t *f) {
    ...
    if (!ata_write_sector(lba, sector_buf))
        return false;
    memset(f, 0, sizeof(fat16_file_t));
    f->entry = entries[idx];
    f->dir_entry_lba = lba;
...
bool fat16_close(fat16_file_t *f) {
    if (!f->open)
        return false;
    if (!ata_read_sector(f->dir_entry_lba, sector_buf))
        return false;
    dir_entry_t *entries = (dir_entry_t *)sector_buf;
    ...
    if (!ata_write_sector(f->dir_entry_lba, sector_buf))
        return false;
    f->open = false;
    return true;
}
...
bool fat16_rename(const char *path, const char *new_name) {
    ...
    dir_entry_t tmp;
    if (fat16_find_in_dir(dir_cluster, new83, &tmp))
        return false;
...
bool fat16_move_file(const char *src_path, const char *dest_path) {
    ...
    if (!ata_read_sector(dst_lba, sector_buf))
        return false;
    ...
    if (!ata_write_sector(dst_lba, sector_buf))
        return false;
    ...
    if (!loc.found)
        return false;
    if (!ata_read_sector(loc.lba, sector_buf))
        return false;
...
bool fat16_move_dir(const char *src_path, const char *dest_path) {
    ...
    dir_entry_t *src_entries = (dir_entry_t *)sector_buf;
    src_entries[loc.idx].name[0] = DIR_ENTRY_FREE;
    if (!ata_write_sector(loc.lba, sector_buf))
        return false;

    uint32_t dotdot_lba = cluster_to_lba(src_entry.cluster_lo);
    if (!ata_read_sector(dotdot_lba, sector_buf))
        return true;
