...
static inline bool strbuf_append(strbuf_t *sb, const char *s) {
    ...
    if (copy > 0) {
        for (size_t i = 0; i < copy; i++) sb->buf[sb->len + i] = s[i];
        sb->len += copy;
        sb->buf[sb->len] = '\0';
    }
    if (slen > avail) sb->overflow = true;
    return !sb->overflow;
}
...
static inline const char *strbuf_str(const strbuf_t *sb) {
    return sb->buf;
}
static inline size_t strbuf_len(const strbuf_t *sb) {
    return sb->len;
}
