...
// content-relative coords to screen coords
...
static void draw_button(widget *wg, int ax, int ay) {
    widget_button *b = &wg->as.button;
    fill_rect(ax, ay, wg->w, wg->h, b->bg_color);
    draw_rect(ax, ay, wg->w, wg->h, b->border_color);
    int tx = ax + wg->w / 2 - (int)(strlen(b->label) * 3);
    int ty = ay + wg->h / 2 - 3;
    draw_string(tx, ty, b->label, b->fg_color, 2);
}

static void draw_label(widget *wg, int ax, int ay) {
    widget_label *l = &wg->as.label;
    draw_string(ax, ay, l->text, l->color, l->font_size);
}

static void draw_checkbox(widget *wg, int ax, int ay) {
    widget_checkbox *cb = &wg->as.checkbox;
    fill_rect(ax, ay, 10, 10, 0xFF);
    draw_rect(ax, ay, 10, 10, 0x00);
    if (cb->checked)
        draw_string(ax + 2, ay + 2, "x", 0x00, 2);
    draw_string(ax + 14, ay + 2, cb->label, 0x00, 2);
}

static void draw_textbox(widget *wg, int ax, int ay) {
    widget_textbox *tb = &wg->as.textbox;
    unsigned char border = tb->focused ? 0x01 : 0x00;
    fill_rect(ax, ay, wg->w, wg->h, 0xFF);
    draw_rect(ax, ay, wg->w, wg->h, border);
    draw_string(ax + 2, ay + 2, tb->buf, 0x00, 2);
    if (tb->focused) {
        int cx = ax + 2 + tb->len * 5;
        if (cx < ax + wg->w - 4)
            draw_string(cx, ay + 2, "|", 0x00, 2);
    }
}
...
static void draw_dropdown(widget *wg, int ax, int ay) {
    ...
    fill_rect(ax, ay, wg->w, wg->h, 0xFF);
    draw_rect(ax, ay, wg->w, wg->h, 0x00);
    if (dd->selected >= 0 && dd->selected < dd->item_count)
        draw_string(ax + 2, ay + 2, dd->items[dd->selected], 0x00, 2);
    draw_string(ax + wg->w - 8, ay + 2, dd->open ? "^" : "v", 0x00, 2);

    if (dd->open) {
        for (int i = 0; i < dd->item_count; i++) {
            int iy = ay + wg->h * (i + 1);
            unsigned char bg = (i == dd->selected) ? 0xC8 : 0xFF;
            fill_rect(ax, iy, wg->w, wg->h, bg);
            draw_rect(ax, iy, wg->w, wg->h, 0x00);
            draw_string(ax + 2, iy + 2, dd->items[i], 0x00, 2);
        }
    }
}
...
static void draw_menu(widget *wg, int ax, int ay) {
    ...
    fill_rect(ax, ay, wg->w, wg->h, 0xD0);
    draw_rect(ax, ay, wg->w, wg->h, 0x00);
    draw_string(ax + 2, ay + 2, "Menu", 0x00, 2);

    if (m->open) {
        for (int i = 0; i < m->item_count; i++) {
            int iy = ay + wg->h * (i + 1);
            fill_rect(ax, iy, wg->w, wg->h, 0xFF);
            draw_rect(ax, iy, wg->w, wg->h, 0x00);
            draw_string(ax + 4, iy + 2, m->items[i], 0x00, 2);
        }
    }
}
