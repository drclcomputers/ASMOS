...
static void grid_pos_local(const icon_view_t *v, int slot, int *out_x,
                           int *out_y) {
...
static inline int abs_x(const icon_view_t *v, int lx) {
    return v->origin_x + lx;
}
static inline int abs_y(const icon_view_t *v, int ly) {
    return v->origin_y + ly;
}
...
void icon_view_init(icon_view_t *v, int origin_x, int origin_y, int area_w,
                    int area_h) {
...
void icon_view_set_origin(icon_view_t *v, int origin_x, int origin_y,
...
int icon_view_add(icon_view_t *v, const char *label, icon_action_t on_launch,
                  int x, int y) {
    int idx = -1;
    for (int i = 0; i < MAX_ICONS_PER_VIEW; i++) {
        if (!v->icons[i].used) {
...
void icon_view_remove(icon_view_t *v, int idx) {
    if (idx < 0 || idx >= MAX_ICONS_PER_VIEW)
        return;
    if (v->_drag_idx == idx)
        v->_drag_idx = -1;
...
void icon_view_update(icon_view_t *v, bool blocked) {
    ...
    if (v->_drag_idx >= 0) {
        ...
        if (mouse.left) {
            ...
            if (new_ly < 0)
                new_ly = 0;
            if (new_lx + ICON_W > v->area_w)
                new_lx = v->area_w - ICON_W;
            if (new_ly + ICON_H > v->area_h)
                new_ly = v->area_h - ICON_H;
    ...
    for (int i = 0; i < MAX_ICONS_PER_VIEW; i++) {
        ...
        bool hit = mouse.x >= ax && mouse.x < ax + ICON_W && mouse.y >= ay &&
                   mouse.y < ay + ICON_H;
    ...
    if (mouse.left_clicked) {
        bool inside_view =
            mouse.x >= v->origin_x && mouse.x < v->origin_x + v->area_w &&
            mouse.y >= v->origin_y && mouse.y < v->origin_y + v->area_h;
        if (inside_view) {
            for (int i = 0; i < MAX_ICONS_PER_VIEW; i++)
...
void desktop_icon_remove(int idx) { icon_view_remove(&s_desktop_view, idx); }
...
void desktop_icons_draw(void) { icon_view_draw(&s_desktop_view); }
