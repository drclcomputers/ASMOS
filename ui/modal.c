...
static bool btn_hit(int bx, int by) {
    return mouse.left_clicked && mouse.x >= bx && mouse.x < bx + MODAL_BTN_W &&
...
static void draw_wrapped(const char *msg, int x, int y, int max_chars,
    ...
    while (start < msg_len && line_num < 3) {
        ...
        int nl_pos = start;
        while (nl_pos < start + max_chars && nl_pos < msg_len &&
               msg[nl_pos] != '\n')
            nl_pos++;
...
void modal_draw(void) {
    ...
    int pi = 0, ti = 0, fi = 0;
    while (pfx[pi] && fi < 62)
        full_title[fi++] = pfx[pi++];
    while (s.title[ti] && fi < 62)
        full_title[fi++] = s.title[ti++];
    full_title[fi] = '\0';

    draw_string(MODAL_X + 4, MODAL_Y + 4, full_title, title_color(), 2);

    draw_wrapped(s.message, MODAL_X + 8, MODAL_Y + 20, 36, 10, BLACK);

    draw_line(MODAL_X, MODAL_BTN_Y - 5, MODAL_X + MODAL_W - 1, MODAL_BTN_Y - 5,
              DARK_GRAY);
